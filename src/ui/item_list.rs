...
pub fn draw_item_list(f: &mut Frame, app: &mut App, area: Rect) {
    ...
    let max_indicator_width = slide_type_indicators
        .iter()
    ...
    let item_list: Vec<ListItem> = app
        ...
        .map(|(i, item)| {
            ...
            let (type_indicator, type_color) = slide_type_indicators
                .iter()
                .find(|(st, _, _)| *st == slide_type)
                .map_or(("[ Text ]", Color::Blue), |(_, text, color)| {
                    (*text, *color)
                });
            let padding =
                " ".repeat(max_indicator_width.saturating_sub(type_indicator.chars().count()) + 1);
            ...
            let base_fg = if is_focused && is_selected {
                focused_fg
            } else {
                default_fg
            };
            let modifier = if is_focused && is_selected {
                Modifier::BOLD
            } else {
                Modifier::empty()
            };
            ...
            if is_ignored {
                ...
                status_display_style = Style::default()
                    .fg(Color::DarkGray)
                    .add_modifier(Modifier::CROSSED_OUT);
    ...
    let items_list_widget =
        List::new(item_list).block(create_titled_block("Items", items_is_focused));
    ...
    let files_list =
        List::new(files).block(create_titled_block("Matching Files", files_is_focused));

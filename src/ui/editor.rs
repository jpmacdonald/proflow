...
pub fn draw_editor(f: &mut Frame, app: &mut App, area: Rect) {
    ...
    let title = match (&app.current_slide_type, &app.current_scripture_header) {
        (SlideType::Scripture, Some(header)) => format!(
            "Editor [{}] │ {}",
            app.current_slide_type.name(),
            header.display()
        ),
        _ => format!("Editor [{}]", app.current_slide_type.name()),
    };

    let border_color = if app.editor_side_pane_focused {
        Color::DarkGray
    } else {
        Color::Yellow
    };
    ...
    for vl in &visual_lines[start_visual..end_visual] {
        ...
        styled_content.push(Line::from(style_visual_line(
            vl,
            selection_bounds,
            base_style,
            base_bg,
...
fn style_visual_line(
    ...
    let selection_highlight_style = Style::default()
        ...
        if !rest_of_line.is_empty() {
            ...
            if let Some((start_y, start_x, end_y, end_x)) = selection_bounds {
                if content_y >= start_y && content_y <= end_y {
                    ...
                    let seg_start = if content_y == start_y {
                        start_x.saturating_sub(marker_len)
                    } else {
                        0
                    };
                    let seg_end = if content_y == end_y {
                        end_x.saturating_sub(marker_len)
                    } else {
                        rest_of_line.len()
                    };
            ...
            spans.push(Span::styled(
                rest_of_line.to_string(),
                base_style.bg(base_bg_color),
            ));
        }
        return spans;
    }
    ...
    if let Some((start_y, start_x, end_y, end_x)) = selection_bounds {
        if content_y >= start_y && content_y <= end_y {
            ...
            let sel_end_in_line = if content_y == end_y {
                end_x
            } else {
                usize::MAX
            };
            ...
            let seg_sel_start = sel_start_in_line
                .saturating_sub(char_offset)
                .min(line_content.len());
            let seg_sel_end = sel_end_in_line
                .saturating_sub(char_offset)
                .min(line_content.len());
...
fn draw_wrap_guide(f: &mut Frame, app: &App, area: Rect) {
    ...
    for y in 0..area.height {
        ...
        f.buffer_mut()
            .set_string(pos, area.top() + y, ch, Style::default().fg(Color::Cyan));
    }
}

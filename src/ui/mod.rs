...
fn draw_command_bar(f: &mut Frame, app: &App, area: Rect) {
    ...
    let border_color = if app.file_search_active {
        Color::Cyan
    } else {
        Color::Yellow
    };
    ...
    if app.mode == AppMode::Editor && app.editor.is_command_mode {
        ...
        f.set_cursor(
            inner_area.left() + app.editor.command_buffer.len() as u16 + 2,
            inner_area.top(),
        );
    } else if app.file_search_active {
...
fn draw_status_message(f: &mut Frame, message: &str) {
    ...
    let width = message.width().saturating_add(6).min(max_width).max(50) as u16;
    ...
    match app.mode {
        ...
        AppMode::ItemList => {
            lines.extend([
                ...
                ("Del/Bksp/Space", "Toggle ignore item", false),
                ("c or e", "Edit item (load .pro or create)", false),
...
fn draw_version_picker(f: &mut Frame, app: &App) {
    ...
    let version_lines: Vec<Line> = versions
        .iter()
        .enumerate()
        .map(|(i, v)| {

...
- File matching: normalization + fuzzy scoring with hymn-number detection, composite title handling, liturgical boosts, and remembered per-item selections.
...
- Files pane: Enter selects file for the current item (marks complete, records preference).
...
- `src/planning_center/` provides the API client and data models.
- `src/utils/file_matcher.rs` indexes `.pro` files and performs scoring/boosting with selection history.
...
- Add caching for the file index and matching results to reduce cold-start cost.
- Improve item filtering/help modals and error surfacing.
